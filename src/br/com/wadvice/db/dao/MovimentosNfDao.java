...
public class MovimentosNfDao extends DefaultDao {
	...
	private String sqlCreateProdutosDetalhesDao = "CREATE TABLE WAD_LINX_MOV_NF (" + 
												"   PORTAL NUMBER, " + 
	...
	private String nomeTabelaProdutosDetalhesDao = "WAD_LINX_MOV_NF";
	...
	public void gravarLista(List<MovimentosNfModel> movimentos) {
		...
		for (MovimentosNfModel movimento : movimentos) {
			if(movimento.getIdentificador() != null && !movimento.getIdentificador().isEmpty()) {
				try {
					...
					System.out.println("Mov: "+ movimento.getIdentificador());
					e.printStackTrace();
				}
			}
		}
...
//		String query = "UPDATE WAD_LINX_MOV_NF MOV " + 
//				"	SET MOV.DATA_ULT_SINC = SYSDATE " + 
//				"WHERE MOV.PORTAL = ?";
//		PreparedStatement stmt = instance.prepareStatement(query);
